...
!***********************************************************************
! The full program was developed thinking in:    Hartree Atomic Units  !
...
!********                      INPUT DATA                       ********
!-----------------------------------------------------------------------
! We will obtain the basis set, molecule over charge, number of atoms, !
! units of cooridantes and an info flag if something is wrong written. !
! Then we will read the atoms an its coordiantes.                      !
...
!********                      BASIS SET                        ********
!-----------------------------------------------------------------------
! NOTE: $d_{i,j}$ values are already normalized, IMPORTANT to know how !
! the program will compute the integrals.                              !
!-----------------------------------------------------------------------

!               know the maximum values to allocate the memory propertly
CALL BASIS_MAX(basis_set, na, max_bf, max_prim, max_zeta)
...
!-----------------------------------------------------------------------
  ...
  DEALLOCATE(n_pri_bf); ALLOCATE(n_pri_bf(bf))         !      ⬇️
                                                       !      ⬇️
  n_pri_bf(:) = n_pri_bf_aux(:)                        !      ⬇️
                                                       !      ⬇️
  DEALLOCATE(n_pri_bf_aux)                             !   Cleaned it  !
!-----------------------------------------------------------------------
...
C(:,:) = 0.d0; Pb(:,:)= 1.d0                                                                !just in case the RAM has something else

!-----------------------------------------------------------------------------------------------------------------------------------
!********                              Here start the loop until the threshold is less than $10^{-10}$                      ********
...
!-----------------------------------------------------------------------
! If all above was done correctly the work of this program was done,   !
! we just need to write it files, DANKE SCHÖN                          !
